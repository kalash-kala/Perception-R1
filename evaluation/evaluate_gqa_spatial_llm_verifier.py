#!/usr/bin/env python3
"""
Evaluate Qwen2.5-VL on GQA spatial open-text task using LLM-as-Judge verification.

This script:
  1. Loads a GQA spatial open-text parquet (with open-ended questions and caption ground truths).
  2. Runs inference with Qwen2.5-VL to generate answers.
  3. Extracts the answer from <answer>...</answer> format.
...
Usage:
  ...
  # 2. Run evaluation:
  python3 /home/kalashkala/Perception-R1/evaluation/evaluate_gqa_spatial_llm_verifier.py \
    --model_path /home/kalashkala/Models/Qwen2.5-VL-3B-Instruct \
    --data_path /home/kalashkala/Datasets/GQA/gqa_spatial_val.parquet \
    --judge_api_base http://localhost:8000/v1 \
    --judge_model /home/kalashkala/Models/Meta-Llama-3.1-8B-Instruct \
    --name gqa_spatial_eval
"""
...
# ---------------------------------------------------------------------------
# LLM-as-a-Judge: Semantic Equivalence Evaluation for Open-Text GQA Spatial
# ---------------------------------------------------------------------------
...
def score_prediction(prediction, ground_truth_str, question, judge):
    ...
    """
    # 1. Check format — extract answer from <answer> tags
    # The prompt format expects <think>...</think> and <answer>...</answer>
    answer_match = re.search(r'<answer>(.*?)</answer>', prediction, re.DOTALL)
    if answer_match:
        predicted_answer = answer_match.group(1).strip()
    else:
        # If no <answer> tags, we mark as no_format
        return -1.0, "no_format", None, "Missing <answer></answer> format"
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Evaluate Qwen2.5-VL on GQA spatial open-text task with LLM-as-Judge"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output_dir", type=str, default="results/gqa_spatial_llm_verifier_eval",
...
def main():
    ...
    print("=" * 60)
    print("Starting GQA Spatial Evaluation (LLM Verifier)")
    ...
    with open(detail_log_path, "w") as detail_file:
        for index, row in tqdm(df.iterrows(), total=len(df)):
            try:
                ...
                # If prompt is a list of dicts (messages), use deepcopy
                if isinstance(raw_prompt, list):
                    messages = copy.deepcopy(list(raw_prompt))
                else:
                    # If it's a string, we might need a different handling, 
                    # but typically VERL uses message lists
                            ...
                            if isinstance(orig_content, str):
                                ...
                                for item in orig_content:
                                    if item['type'] == 'text':
                                        ...
                                        # Also ensure image path is correct if already a list
                                        # (Not strictly necessary if we replace content entirely)
                ...
                if 'reward_model' in row:
                    ...
                    if isinstance(rm_data, dict):
                        ...
                        try:
                            # Try literal_eval or json
                            rm_dict = ast.literal_eval(rm_data)
                            ground_truth_str = str(rm_dict.get('ground_truth', 'unknown'))
                        except Exception:
                            try:
                                rm_dict = json.loads(rm_data)
                                ground_truth_str = str(rm_dict.get('ground_truth', 'unknown'))
                            except Exception:
                                pass

                # Extract question from the user message for the judge
                question = "unknown"
                found_q = False
                for msg in row['prompt'] if isinstance(row['prompt'], list) else messages:
                    if msg['role'] == 'user':
                        q_text = msg['content']
                        if isinstance(q_text, list):
                            for item in q_text:
                                if item['type'] == 'text':
                                    question = item['text'].replace("<image>\n", "").replace("<image>", "").strip()
                                    found_q = True
                                    break
                        else:
                            question = q_text.replace("<image>\n", "").replace("<image>", "").strip()
                            found_q = True
                        if found_q: break
